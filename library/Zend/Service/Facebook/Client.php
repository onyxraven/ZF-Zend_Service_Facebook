...
abstract class Zend_Service_Facebook_Client extends Zend_Http_Client
{
     ...
     * @return object
     */
    abstract public function makeRequest($url, $args = array(), $http_method = Zend_Http_Client::GET, $files = array(), $response_format = Zend_Service_Facebook::RESPONSE_JSON);
    ...
    protected function parseResponse(Zend_Http_Response $response, $response_format = Zend_Service_Facebook::RESPONSE_JSON)
    {
        ...
        try {
            ...
            throw new Zend_Service_Facebook_Exception('Could not parse response: ' . $e->getMessage(), (200000 + $e->getCode()));
        }
        if (empty($result)) {
        ...
        if ($response->isError()) {
            throw new Zend_Service_Facebook_Exception('HTTP Response: ' . $response->getMessage() . ' (' . $responseBody . ')', (10000 + $response->getStatus()));
        }
