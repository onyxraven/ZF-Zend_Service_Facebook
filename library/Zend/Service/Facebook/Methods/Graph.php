 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class Zend_Service_Facebook_Methods_Graph 
extends Zend_Service_Facebook_Methods 
{

    public function get($id, $relationship = null, $parameters = array(), 
                        $access_token = true) 
    {
        $url = $id;
        if ($relationship) $url .= '/' . $relationship;
        if ($access_token) $parameters['access_token'] = $access_token;

        return $this->makeRequest($url, $parameters, Zend_Http_Client::GET);
    }

    public function post($id, $relationship, $args = array(), 
                         $access_token = true) 
    {
        $url = $id;
        if ($relationship) $url .= '/' . $relationship;
        ...
        return $this->makeRequest($url, $args, Zend_Http_Client::POST);
    }

    public function delete($id, $relationship = null,  
                           $access_token = true) 
    {
        $url = $id;
        if ($relationship) $url .= '/' . $relationship;
