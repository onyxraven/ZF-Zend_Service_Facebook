 ...
 * @license http://framework.zend.com/license/new-bsd New BSD License
 */
class Zend_Service_Facebook_Client_Rest extends Zend_Service_Facebook_Client
{
     ...
     * @var string $api
     */
    protected static $api_url = 'http://api.new.facebook.com/restserver.php';
     ...
     *            optional session key to init with
     * @param string $base_domain
     *            domain to set cookies on
     *            
     * @return void
     */
    public function __construct($key, $secret, $session_key = null, $base_domain = null)
    {
        $this->setApiKey($key);
     ...
     *      
     * @param array $source
     *            one or more sources to pull from (null is all)
     * @param int $timeout
     *            oldest acceptable params
    ...
    public function getAutoFacebookParams(array $source = null, $timeout = 172800)
    {
        ...
        if (! $source || $source == self::PARAM_SOURCE_POST || ! empty($source[self::PARAM_SOURCE_POST])) {
            $params = array_merge($params, $this->getFacebookParams($_POST, self::$val_prefix[self::PARAM_SOURCE_POST], $timeout));
        }
        if (! $params && (! $source || $source == self::PARAM_SOURCE_GET || ! empty($source[self::PARAM_SOURCE_GET]))) {
            $params = array_merge($params, $this->getFacebookParams($_GET, self::$val_prefix[self::PARAM_SOURCE_GET], $timeout));
            $params = array_merge($params, $this->getFacebookParams($_POST, self::$val_prefix[self::PARAM_SOURCE_GET_POST], $timeout));
        }
        if (! $params && (! $source || $source == self::PARAM_SOURCE_COOKIE || ! empty($source[self::PARAM_SOURCE_COOKIE]))) {
            $params = array_merge($params, $this->getFacebookParams($_COOKIE, $this->getApiKey()));
     ...
     *
     * @param array $source
     *            which sources to try to look at
     * @param int $timeout            
     * @return self
     */
     ...
     *            as prefix for params
     * @param int $timeout
     *            max age of timestamp
    ...
    public function getFacebookParams(array $params, $namespace, $timeout = null)
    {
        ...
        if ($timeout && (! isset($params[$prefix . 'time']) || time() - $params[$prefix . 'time'] > $timeout)) {
            return array();
        }
    ...
    public function clearCookies()
    {
        ...
        foreach ($cookies as $name) {
            $ckname = $this->getApiKey() . '_' . $name;
            setcookie($ckname, false, time() - 3600, '', $this->getBaseDomain());
